...
exports.upsertSeatCategorySchema = exports.createEventSchema = void 0;
...
exports.listSeatCategories = listSeatCategories;
exports.upsertSeatCategory = upsertSeatCategory;
...
exports.createEventSchema = zod_1.z.object({
    ...
    end_date: zod_1.z.string().min(1, 'end_date is required'),
    capacity: zod_1.z.union([zod_1.z.number(), zod_1.z.string().transform((v) => Number(v))]).optional(),
    banner_url: zod_1.z.string().optional(),
    status: zod_1.z.enum(['draft', 'published', 'ended']).optional(),
});
exports.upsertSeatCategorySchema = zod_1.z.object({
    catId: zod_1.z.string().optional(),
    name: zod_1.z.string().min(1, 'Category name is required'),
    price: zod_1.z.union([zod_1.z.number(), zod_1.z.string().transform((v) => Number(v))]),
    rows: zod_1.z.union([zod_1.z.array(zod_1.z.string()), zod_1.z.string().transform((v) => [v])]),
    cols: zod_1.z.union([zod_1.z.number(), zod_1.z.string().transform((v) => Number(v))]),
    color: zod_1.z.string().optional(),
});
/* ─── helpers ─────────────────────────────────────────────── */
function expireLockedSeats(eventId) {
    const now = Date.now();
...
function rebuildSeatsFromCategories(eventId) {
    // Keep sold seats to avoid orphan tickets
    const soldSeats = dataStore_1.dataStore.seats.filter((s) => s.event_id === eventId && s.status === 'sold');
    dataStore_1.dataStore.seats = dataStore_1.dataStore.seats.filter((s) => s.event_id !== eventId || s.status === 'sold');
    const categories = dataStore_1.dataStore.seatCategories.filter((c) => c.event_id === eventId);
    for (const cat of categories) {
        for (const rowLabel of cat.rows) {
            for (let col = 1; col <= cat.cols; col++) {
                ...
                dataStore_1.dataStore.seats.push({
                    id: seatId,
                    event_id: eventId,
                    row: rowLabel,
                    number: col,
...
async function listEvents(req, res) {
    ...
    let result = dataStore_1.dataStore.events.filter((e) => e.tenant_id === tenantId && e.status !== 'deleted');
    ...
    res.json(apiResponse_1.ApiResponse.paginated(paginated, pageNum, limitNum, total));
}
/* ─── admin: all events across tenants ─────────────────────── */
async function listAllEvents(req, res) {
    const events = dataStore_1.dataStore.events.filter((e) => e.status !== 'deleted');
    res.json(apiResponse_1.ApiResponse.success(events, `${events.length} events across all tenants`));
}
/* ─── public: event detail ────────────────────────────────── */
async function getEventById(req, res) {
    const id = req.params.id;
    const event = dataStore_1.dataStore.events.find((e) => e.id === id && e.status !== 'deleted');
    if (!event) {
        res.status(404).json(apiResponse_1.ApiResponse.error('Event not found', 404));
        return;
    }
    // Attach computed stats
    const allSeats = dataStore_1.dataStore.seats.filter((s) => s.event_id === id);
...
async function getEventSeats(req, res) {
    const id = req.params.id;
    expireLockedSeats(id);
    const eventSeats = dataStore_1.dataStore.seats.filter((s) => s.event_id === id);
    res.json(apiResponse_1.ApiResponse.success({
        event_id: id,
...
async function createEvent(req, res) {
    ...
    const { name, description, category, location, venue_name, start_date, end_date, capacity, banner_url, status = 'draft', } = req.body;
    if (!name || !start_date || !end_date || !location) {
        res.status(400).json(apiResponse_1.ApiResponse.error('name, location, start_date, end_date are required', 400));
        return;
    }
    const newEvent = {
    ...
    dataStore_1.dataStore.events.push(newEvent);
    ...
    dataStore_1.dataStore.seatCategories.push(...defaultCategories);
    // Generate initial seat layout
    rebuildSeatsFromCategories(newEvent.id);
    // Recalculate price_min/max
    const cats = dataStore_1.dataStore.seatCategories.filter((c) => c.event_id === newEvent.id);
    if (cats.length) {
        newEvent.price_min = Math.min(...cats.map((c) => c.price));
        newEvent.price_max = Math.max(...cats.map((c) => c.price));
    }
    res.status(201).json(apiResponse_1.ApiResponse.success(newEvent, 'Event created successfully'));
}
...
async function updateEvent(req, res) {
    ...
    if (actor.role === 'organizer' && event.organizer_id !== actor.userId) {
        res.status(403).json(apiResponse_1.ApiResponse.error('You can only edit your own events', 403));
        return;
    }
    ...
    res.json(apiResponse_1.ApiResponse.success(event, 'Event updated successfully'));
}
...
async function deleteEvent(req, res) {
    ...
    if (actor.role === 'organizer' && event.organizer_id !== actor.userId) {
        res.status(403).json(apiResponse_1.ApiResponse.error('You can only delete your own events', 403));
        return;
    }
    // Soft delete
    event.status = 'deleted';
    res.json(apiResponse_1.ApiResponse.success({ id }, 'Event deleted successfully'));
}
...
async function uploadBanner(req, res) {
    ...
    if (actor.role === 'organizer' && event.organizer_id !== actor.userId) {
        res.status(403).json(apiResponse_1.ApiResponse.error('Forbidden', 403));
        return;
    }
    let finalBannerUrl = '';
    if (req.file) {
    ...
    res.json(apiResponse_1.ApiResponse.success({ banner_url: event.banner_url }, 'Banner updated successfully'));
}
/* ─── organizer: list seat categories ────────────────────── */
async function listSeatCategories(req, res) {
    const id = req.params.id;
    const categories = dataStore_1.dataStore.seatCategories.filter((c) => c.event_id === id);
    res.json(apiResponse_1.ApiResponse.success(categories, 'Seat categories retrieved'));
}
/* ─── organizer: upsert seat category ────────────────────── */
async function upsertSeatCategory(req, res) {
    const id = req.params.id;
    const actor = req.user;
    const event = dataStore_1.dataStore.events.find((e) => e.id === id && e.status !== 'deleted');
    if (!event) {
    ...
    if (!name || !price || !rows || !cols) {
        res.status(400).json(apiResponse_1.ApiResponse.error('name, price, rows (array), cols are required', 400));
        return;
    }
    let cat = catId ? dataStore_1.dataStore.seatCategories.find((c) => c.id === catId && c.event_id === id) : undefined;
    if (cat) {
        cat.name = name;
        ...
        cat = newCat;
        dataStore_1.dataStore.seatCategories.push(newCat);
    }
    // Recalculate price_min/max on event
    const allCats = dataStore_1.dataStore.seatCategories.filter((c) => c.event_id === id);
    event.price_min = Math.min(...allCats.map((c) => c.price));
    event.price_max = Math.max(...allCats.map((c) => c.price));
    res.json(apiResponse_1.ApiResponse.success(cat, catId ? 'Category updated' : 'Category added'));
}
/* ─── organizer: delete seat category ────────────────────── */
async function deleteSeatCategory(req, res) {
    const id = req.params.id;
    ...
    const idx = dataStore_1.dataStore.seatCategories.findIndex((c) => c.id === catId && c.event_id === id);
    if (idx === -1) {
        res.status(404).json(apiResponse_1.ApiResponse.error('Category not found', 404));
        return;
    }
    dataStore_1.dataStore.seatCategories.splice(idx, 1);
    // Recalculate
    const remaining = dataStore_1.dataStore.seatCategories.filter((c) => c.event_id === id);
    if (remaining.length) {
        event.price_min = Math.min(...remaining.map((c) => c.price));
        event.price_max = Math.max(...remaining.map((c) => c.price));
    }
    res.json(apiResponse_1.ApiResponse.success({ catId }, 'Category deleted'));
}
...
async function regenerateSeats(req, res) {
    ...
    const newSeats = dataStore_1.dataStore.seats.filter((s) => s.event_id === id);
    res.json(apiResponse_1.ApiResponse.success({ total_seats: newSeats.length, event_id: id }, 'Seat layout regenerated from categories'));
}
/* ─── organizer: my events ────────────────────────────────── */
async function listMyEvents(req, res) {
    const actor = req.user;
    const { status } = req.query;
    let result = dataStore_1.dataStore.events.filter((e) => e.organizer_id === actor.userId &&
    ...
    const enriched = result.map((e) => {
        const seats = dataStore_1.dataStore.seats.filter((s) => s.event_id === e.id);
