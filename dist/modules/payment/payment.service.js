...
exports.isMidtransConfigured = isMidtransConfigured;
exports.createMidtransSnapToken = createMidtransSnapToken;
exports.verifyMidtransSignature = verifyMidtransSignature;
exports.createOrderService = createOrderService;
exports.issueTicketsForOrder = issueTicketsForOrder;
exports.processPaymentService = processPaymentService;
exports.mapMidtransStatus = mapMidtransStatus;
exports.processWebhookService = processWebhookService;
...
async function createMidtransSnapToken(params) {
    const authHeader = 'Basic ' + Buffer.from(`${env_1.env.MIDTRANS_SERVER_KEY}:`).toString('base64');
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.token) {
                        ...
                        const msg = parsed.error_messages?.join(', ') ||
...
function verifyMidtransSignature(params) {
    if (!params.signatureKey)
        return false;
    const raw = `${params.orderId}${params.statusCode}${params.grossAmount}${env_1.env.MIDTRANS_SERVER_KEY}`;
...
// ─────────────────────────────────────────────────────────────────────────────
function buildSimulationUrl(orderId, amount) {
    const base = env_1.env.CORS_ORIGIN || 'http://localhost:3000';
...
async function createOrderService(input) {
    const { event_id, seat_ids, payment_gateway, customer_name, customer_email, idempotency_key, user_id, tenant_id } = input;
    ...
    for (const seatId of seat_ids) {
        ...
        if (seat.status === 'sold') {
            throw Object.assign(new Error(`Seat ${seat.row}-${seat.number} (${seat.category}) is already sold`), { statusCode: 409 });
        }
    ...
    if (isMidtransConfigured()) {
        try {
            ...
            logger_1.logger.info(`[PaymentService] Midtrans Snap token created for order ${orderId}`);
        }
        catch (err) {
            gatewayError = err.message;
            logger_1.logger.warn(`[PaymentService] Midtrans Snap failed: ${gatewayError} — falling back to simulation`);
        }
    }
    ...
    for (const seatId of order.seat_ids) {
        ...
        if (isRedisReady()) {
            if (seat.locked_by_user_id === userId) {
                ...
                await redis_1.redis.del(lockKey).catch(() => { }); // admin force-release
            }
        }
        // Update seat status
        seat.status = 'sold';
        ...
        }, tenantId).catch((err) => logger_1.logger.warn('[PaymentService] Failed to publish ticket.issued', err));
    }
...
// ─────────────────────────────────────────────────────────────────────────────
async function processPaymentService(orderId, userId, tenantId, userRole) {
    const order = dataStore_1.dataStore.orders.find((o) => o.id === orderId);
    if (!order) {
    ...
    if (order.status === 'paid') {
        const existingTickets = dataStore_1.dataStore.tickets.filter((t) => t.order_id === orderId);
        return { order, tickets: existingTickets };
    }
    if (order.status === 'failed' || order.status === 'expired') {
        throw Object.assign(new Error(`Order cannot be paid — status is '${order.status}'`), { statusCode: 409 });
    }
    ...
    }, tenantId).catch((err) => logger_1.logger.warn('[PaymentService] Failed to publish order.paid', err));
    ...
    logger_1.logger.info(`[PaymentService] Simulation payment processed for order ${orderId} — ` +
...
async function processWebhookService(payload) {
    ...
    if (isMidtransConfigured()) {
        ...
        if (!valid) {
            logger_1.logger.warn(`[PaymentService/Webhook] Invalid signature for order ${order_id}`);
            throw Object.assign(new Error('Invalid webhook signature'), { statusCode: 401 });
        }
    }
    else {
        logger_1.logger.debug('[PaymentService/Webhook] Dev mode — skipping Midtrans signature check');
    }
    // ── 2. Locate order ──────────────────────────────────────────────────────
    const order = dataStore_1.dataStore.orders.find((o) => o.id === order_id);
    if (!order) {
        logger_1.logger.warn(`[PaymentService/Webhook] Order not found: ${order_id}`);
    ...
    if (transaction_id && order.gateway_ref === transaction_id && order.status === 'paid') {
        logger_1.logger.info(`[PaymentService/Webhook] Duplicate webhook for txn ${transaction_id} — ignored`);
    ...
    logger_1.logger.info(`[PaymentService/Webhook] Order ${order_id}: ${order.status} → ${newStatus} ` +
    ...
    if (shouldIssueTickets) {
        ...
        (0, publisher_1.publishEvent)('order.paid', {
            ...
            transaction_id,
        }, tenantId).catch((err) => logger_1.logger.error('[PaymentService/Webhook] Failed to publish order.paid', err));
        ...
        logger_1.logger.info(`[PaymentService/Webhook] Issued ${ticketsIssued} ticket(s) for order ${order_id}`);
    }
