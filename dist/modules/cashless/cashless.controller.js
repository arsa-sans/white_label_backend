...
exports.refundBoothTx = refundBoothTx;
exports.autoRefundJob = autoRefundJob;
exports.getBoothHistory = getBoothHistory;
const dataStore_1 = require("../../database/dataStore");
const apiResponse_1 = require("../../utils/apiResponse");
const publisher_1 = require("../../queue/publisher");
const logger_1 = require("../../utils/logger");
// Helper: Get or create wallet
function getOrCreateWallet(userId, eventId = 'evt-001') {
...
// ─────────────────────────────────────────────────────────────────────────────
async function pairNfc(req, res) {
    ...
    if (!nfc_uid || typeof nfc_uid !== 'string' || nfc_uid.trim() === '') {
        res.status(400).json(apiResponse_1.ApiResponse.error('Valid nfc_uid string is required', 400));
        return;
    }
    const cleanUid = nfc_uid.trim().toUpperCase();
    // Check if NFC UID is already paired to another active wallet
    for (const [wUserId, existingWallet] of Array.from(dataStore_1.dataStore.wallets.entries())) {
        if (existingWallet.nfc_uid === cleanUid && wUserId !== userId) {
            res.status(409).json(apiResponse_1.ApiResponse.error(`NFC UID '${cleanUid}' is already paired to another attendee wallet`, 409));
            return;
        }
    }
    const wallet = getOrCreateWallet(userId);
    wallet.nfc_uid = cleanUid;
    logger_1.logger.info(`[Cashless] Paired NFC UID ${cleanUid} to user ${userId}`);
    res.json(apiResponse_1.ApiResponse.success({
        wallet,
...
// ─────────────────────────────────────────────────────────────────────────────
async function debitBooth(req, res) {
    const { amount, nfc_uid, user_id, reference_id, booth_name = 'F&B Booth #1', items_summary } = req.body;
    if (!amount || typeof amount !== 'number' || amount <= 0) {
        res.status(400).json(apiResponse_1.ApiResponse.error('Valid positive transaction amount is required', 400));
        return;
    }
    if (!reference_id || typeof reference_id !== 'string') {
        res.status(400).json(apiResponse_1.ApiResponse.error('Idempotency reference_id (UUID v4) is required per booth tap', 400));
        return;
    }
    // Check idempotency: if reference_id seen before, return cached transaction
    const existingTx = dataStore_1.dataStore.walletTxs.find((t) => t.id === reference_id || t.description.includes(reference_id));
    if (existingTx) {
        const existingWallet = Array.from(dataStore_1.dataStore.wallets.values()).find((w) => w.id === existingTx.wallet_id);
        res.json(apiResponse_1.ApiResponse.success({
    ...
    if (nfc_uid) {
        const cleanUid = nfc_uid.trim().toUpperCase();
        targetWallet = Array.from(dataStore_1.dataStore.wallets.values()).find((w) => w.nfc_uid === cleanUid);
    }
    else if (user_id) {
        targetWallet = dataStore_1.dataStore.wallets.get(user_id);
    }
    else {
        // Fallback: use logged-in user's wallet
        targetWallet = dataStore_1.dataStore.wallets.get(req.user?.userId);
    }
    if (!targetWallet) {
        res.status(404).json(apiResponse_1.ApiResponse.error(`Wallet not found for ${nfc_uid ? `NFC '${nfc_uid}'` : 'user'}`, 404));
        return;
    }
    // Row lock simulation & Balance check (SKILLS.md § Skill 6)
    if (targetWallet.balance < amount) {
        res.status(402).json(apiResponse_1.ApiResponse.error(`Insufficient balance. Saldo: Rp ${targetWallet.balance.toLocaleString('id-ID')}, Dibutuhkan: Rp ${amount.toLocaleString('id-ID')}`, 402));
        return;
    }
    ...
    dataStore_1.dataStore.walletTxs.unshift(tx);
    logger_1.logger.info(`[Cashless] Booth debit success — ${booth_name}: Rp ${amount} debited from wallet ${targetWallet.id} (ref=${reference_id})`);
    res.json(apiResponse_1.ApiResponse.success({
        wallet: targetWallet,
        transaction: tx,
...
// Auth: authenticate (role: vendor, admin, organizer)
// ─────────────────────────────────────────────────────────────────────────────
async function refundBoothTx(req, res) {
    const { transaction_id, reason = 'Customer refund request' } = req.body;
    if (!transaction_id) {
        res.status(400).json(apiResponse_1.ApiResponse.error('transaction_id is required', 400));
        return;
    }
    const txIndex = dataStore_1.dataStore.walletTxs.findIndex((t) => t.id === transaction_id);
    if (txIndex === -1) {
        res.status(404).json(apiResponse_1.ApiResponse.error('Transaction not found', 404));
        return;
    }
    const origTx = dataStore_1.dataStore.walletTxs[txIndex];
    if (origTx.type !== 'payment') {
        res.status(400).json(apiResponse_1.ApiResponse.error(`Only 'payment' transactions can be refunded (type is '${origTx.type}')`, 400));
        return;
    }
    const wallet = Array.from(dataStore_1.dataStore.wallets.values()).find((w) => w.id === origTx.wallet_id);
    if (!wallet) {
        res.status(404).json(apiResponse_1.ApiResponse.error('Associated wallet not found', 404));
        return;
    }
    // Credit balance back
    ...
    dataStore_1.dataStore.walletTxs.unshift(refundTx);
    res.json(apiResponse_1.ApiResponse.success({
        wallet,
...
// ─────────────────────────────────────────────────────────────────────────────
async function autoRefundJob(req, res) {
    const { event_id = 'evt-001' } = req.body;
    let refundedCount = 0;
    let totalRefundedAmount = 0;
    for (const [, wallet] of Array.from(dataStore_1.dataStore.wallets.entries())) {
        if (wallet.event_id === event_id && wallet.balance > 0) {
            ...
            (0, publisher_1.publishEvent)('refund.processed', {
                wallet_id: wallet.id,
                user_id: wallet.user_id,
                amount: amountToRefund,
                event_id,
            }, req.user?.tenantId || 'tenant-001').catch(() => { });
        }
    }
    logger_1.logger.info(`[Cashless] Auto-refund job completed: ${refundedCount} wallet(s) refunded, total Rp ${totalRefundedAmount}`);
    res.json(apiResponse_1.ApiResponse.success({
        event_id,
...
// Auth: authenticate
// ─────────────────────────────────────────────────────────────────────────────
async function getBoothHistory(req, res) {
    const txs = dataStore_1.dataStore.walletTxs.filter((t) => t.type === 'payment' || t.type === 'refund');
    res.json(apiResponse_1.ApiResponse.success(txs, 'Booth transaction history retrieved'));
}
