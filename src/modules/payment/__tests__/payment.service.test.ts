...
function getAvailableSeat(eventId = 'evt-001') {
  return dataStore.seats.find((s) => s.event_id === eventId && s.status === 'available')!;
}
...
describe('createOrderService', () => {
  ...
  test('throws 409 if seat is already sold', async () => {
    const soldSeat = dataStore.seats.find(
      (s) => s.event_id === DEMO_EVENT_ID && s.status === 'sold'
    )!;
...
describe('issueTicketsForOrder', () => {
  ...
  test('issues ticket and marks seat as sold', async () => {
    ...
    const seat = dataStore.seats.find((s) => s.id === testSeatId);
    expect(seat?.status).toBe('sold');
  });

  test('is idempotent — second call returns same ticket without duplicate', async () => {
    const tickets1 = await issueTicketsForOrder(testOrderId, DEMO_USER_ID, DEMO_TENANT_ID);
    const tickets2 = await issueTicketsForOrder(testOrderId, DEMO_USER_ID, DEMO_TENANT_ID);
  ...
  test('publishes ticket.issued event to RabbitMQ (attempt)', async () => {
    const mockPublish = publishEvent as jest.Mock;
    mockPublish.mockClear();

    await issueTicketsForOrder(testOrderId, DEMO_USER_ID, DEMO_TENANT_ID);
  ...
  test('returns empty array for nonexistent order', async () => {
    const tickets = await issueTicketsForOrder('nonexistent-order', DEMO_USER_ID, DEMO_TENANT_ID);
...
describe('processPaymentService', () => {
  ...
  test('is idempotent — calling twice returns same result (no duplicate tickets)', async () => {
    const result1 = await processPaymentService(
      pendingOrderId,
      DEMO_USER_ID,
      DEMO_TENANT_ID,
      'visitor'
    );
    const result2 = await processPaymentService(
      pendingOrderId,
      DEMO_USER_ID,
      DEMO_TENANT_ID,
      'visitor'
  ...
  test('throws 404 if order not found', async () => {
    await expect(
      processPaymentService('nonexistent-order', DEMO_USER_ID, DEMO_TENANT_ID, 'visitor')
    ).rejects.toMatchObject({ statusCode: 404 });
  });

  test('throws 403 if user is not the order owner', async () => {
    await expect(
      processPaymentService(pendingOrderId, 'other-user-id', DEMO_TENANT_ID, 'visitor')
  ...
  test('throws 409 for already-failed order', async () => {
    ...
    await expect(
      processPaymentService(failedOrderId, DEMO_USER_ID, DEMO_TENANT_ID, 'visitor')
    ).rejects.toMatchObject({ statusCode: 409 });
  });
  ...
  test('publishes order.paid event after successful payment', async () => {
    ...
    await processPaymentService(pendingOrderId, DEMO_USER_ID, DEMO_TENANT_ID, 'visitor');
...
describe('mapMidtransStatus', () => {
  ...
  test('capture + deny → failed, shouldIssueTickets=false', () => {
    expect(mapMidtransStatus('capture', 'deny')).toEqual({
      newStatus: 'failed',
  ...
  test('deny → failed', () => {
    const { newStatus, shouldIssueTickets } = mapMidtransStatus('deny');
    expect(newStatus).toBe('failed');
    expect(shouldIssueTickets).toBe(false);
  });

  test('cancel → failed', () => {
    const { newStatus } = mapMidtransStatus('cancel');
    expect(newStatus).toBe('failed');
  });

  test('refund → failed', () => {
    const { newStatus } = mapMidtransStatus('refund');
    expect(newStatus).toBe('failed');
  });

  test('unknown status → pending (no change)', () => {
    const { newStatus, shouldIssueTickets } = mapMidtransStatus('unknown_status');
...
describe('processWebhookService', () => {
  ...
  let webhookSeatId: string;

  function makeWebhookPayload(
    overrides: Partial<MidtransWebhookPayload> = {}
  ): MidtransWebhookPayload {
  ...
  test('processes settlement webhook — marks order paid, issues tickets', async () => {
    const result = await processWebhookService(makeWebhookPayload());

    expect(result.skipped).toBe(false);
    expect(result.new_status).toBe('paid');
    expect(result.tickets_issued).toBeGreaterThanOrEqual(1);

    const order = dataStore.orders.find((o) => o.id === webhookOrderId);
  ...
  test('is idempotent — duplicate webhook with same transaction_id is skipped', async () => {
    ...
    const result1 = await processWebhookService(makeWebhookPayload({ transaction_id: transactionId }));
    const result2 = await processWebhookService(makeWebhookPayload({ transaction_id: transactionId }));
  ...
  test('returns skipped=true for nonexistent order (avoid retry loop)', async () => {
    const result = await processWebhookService(
      makeWebhookPayload({ order_id: 'nonexistent-order-000' })
    );
  ...
  test('marks order expired for expire transaction_status', async () => {
    const result = await processWebhookService(
      makeWebhookPayload({ transaction_status: 'expire', fraud_status: undefined })
    );
  ...
  test('marks order failed for cancel transaction_status', async () => {
    const result = await processWebhookService(
      makeWebhookPayload({ transaction_status: 'cancel', fraud_status: undefined })
    );
  ...
  test('does not issue tickets on pending webhook', async () => {
    const result = await processWebhookService(
      makeWebhookPayload({ transaction_status: 'pending', fraud_status: undefined })
    );
